...
time='1438203822.293606645';

[status,output] = system(['cat powerclamp.' time '.output | grep HadoopKmeans | tail -n 1 | tr -s \  | cut -d \  -f 4,5']);
...
[status,output] = system(['cat powerclamp.' time '.output | grep Using | cut -d \  -f 2']);
setpoint = dlmread(output(1:end-1),',');

actual = dlmread(['powerclamp.' time '.power'],' ');

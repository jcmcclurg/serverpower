...
time='1438013575.020295221';
[status,output] = system(['cat stress.' time '.output | grep Ran | cut -d \  -f 2,5']);
[multipliers,iterations]=strread(output,'%f %f');
fprintf(1,'Total sqrt operations: %g\n',sum(multipliers.*iterations))

[status,output] = system(['cat stress.' time '.output | grep Taking | cut -d \  -f 4']);
setpoint = dlmread(output(1:end-1),',');

actual = dlmread(['stress.' time '.power'],' ');

...
time='1438018570.370566945';

[status,output] = system(['cat hypervisor_no_power_control.' time '.output | grep HadoopKmeans | tail -n 1 | tr -s \  | cut -d \  -f 4,5']);
...
[status,output] = system(['cat hypervisor_no_power_control.' time '.output | grep Using | cut -d \  -f 2']);

setpoint = dlmread(output(1:end-1),',');
actual = dlmread(['hypervisor_no_power_control.' time '.power'],' ');
...
print('hypervisor_no_power_control.pdf')

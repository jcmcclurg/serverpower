...
Description: 
	...
	prints received freq deviation to stdout
	prints freq and timestamp to file	
	transmits user entered (stdin) bytes
	
compile with the command: gcc getFreq.c rs232.c -Wall -Wextra -o2 -o getFreq
example run to only get freq: sudo ./getFreq -r 500 -p 16 -b 115200 -o test.csv
...
void usage(){
	...
	fprintf(stdout, "Example usage:\n"
		"    sudo ./getFreq -r 500 -p 16 -b 115200 -o test.csv\n");
	fprintf(stdout, "\n");
}

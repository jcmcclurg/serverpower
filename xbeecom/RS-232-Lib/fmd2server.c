...
Description: 
	...
	converts received freq deviation to a power setpoint and prints (stdout) result
...
example run to only get freq without setup: sudo ./fmd2server -r 500 -p 16 -b 115200 -s 0 -o test2.csv
...
int cmdline(int argc, char *argv[]){
	int opt;
	progname = argv[0];
	while ((opt = getopt(argc, argv, "a:p:b:r:M:m:o:s:h")) != -1) {
		switch (opt) {
			case 'a':

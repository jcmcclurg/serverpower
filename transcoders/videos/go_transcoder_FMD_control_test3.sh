...
(unbuffer -p avconv -i $vid_input -r 30 -y $vid_output 2>/dev/null; echo "S"; sleep 1; echo "q") | (sudo $power_monitor -e 150 & pgid2=$!; sudo $transcd_ctrlr -r 500 -p 16 -b 115200 -M 30 -m 16 -a 1 -B 30000 -o $transcd_log; sudo kill -s SIGINT $pgid2; sleep 1; echo "q") | ($integral_ctrlr -s 28 -n 0 -x 1 -t 0.1 -k 0 -d 0 -u 10; sleep 1; echo "q") | (sleep 1; $insert_delays -U -d 0.5 -p $(pgrep avconv))

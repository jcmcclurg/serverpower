...
for experiment in ['cpufreq','rapl']:
	if verbose:
	...
	for server in serverNums:
		for k in range(setpointData[server]["residencyChunks"][j].shape[1]):
			adjusted = False
			for j in range(len(setpointData[server]["residencyChunks"])):
				# Make sure that the data doesn't have all identical values. This messes up the beanplot.
				firstValue = setpointData[server]["residencyChunks"][j][0,k]
				if np.std(setpointData[server]["residencyChunks"][j][:,k]) <= 0.0:
					adjusted = True
					setpointData[server]["residencyChunks"][j][:,k] += np.random.rand(setpointData[server]["residencyChunks"][j].shape[0])/1e9 - (0.5/1.0e9)
					assert(np.std(setpointData[server]["residencyChunks"][j][:,k]) > 0.0)

			if verbose and adjusted:
				print "  Server %d: Had to adjust %s."%(server,residencyDescriptions[k])
	...
	for server in serverNums:
		...
		for j in residencyDescriptionsToPlot:
			...
			try:
				sm.graphics.beanplot([i[:,j] for i in setpointData[server]["residencyChunks"]], ax=ax, jitter=False, plot_opts=plot_opts)
			...
			if counter > len(residencyDescriptionsToPlot) - width:
				ax.set_xticklabels([ formats[experiment]%(multipliers[experiment]*setpointData[server]["setpoints"][k]) for k in range(0,numSetpoints,ticksEvery)],rotation=45)
		...
		fig1.tight_layout()
		if saveFig:
			path = currentExpDir+'/'+experiment+'_server'+str(server)+'_residency_short.png'
			if verbose:
				print "  Wrote: "+path
		...
		sm.graphics.beanplot(setpointData[server]["powerChunks"], ax=ax, jitter=False, plot_opts=plot_opts)
		...
		fig2.tight_layout()
		if saveFig:
			path = currentExpDir+'/'+experiment+'_server'+str(server)+'_power.png'
			if verbose:
				print "  Wrote: "+path
			plt.savefig(path, bbox_inches='tight')
		if not showFig:

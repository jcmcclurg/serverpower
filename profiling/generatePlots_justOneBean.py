...
for experiment in ['cgroups']:
	...
	for server in serverNums:
		...
		for j in residencyDescriptionsToPlot:
			...
			try:
				sm.graphics.beanplot([ setpointData[server]["residencyChunks"][setpointIndex][j] ], ax=ax, jitter=False, plot_opts=plot_opts)
			...
			xtickValues = multipliers[experiment]*np.array(setpointData[server]["setpoints"])
			ax.set_xlabel("Interface setpoint %g %s"%(xtickValues[setpointIndex],units[experiment]))
		...
		sm.graphics.beanplot([ setpointData[server]["powerChunks"][setpointIndex] ], ax=ax, jitter=False, plot_opts=plot_opts)

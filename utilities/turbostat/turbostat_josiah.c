 ...
 * Modified 8/13/2016 by Josiah McClurg (added timestamp feature)
 */
	...
	if (t == &average.threads) {
		if (show_time) {
			if (tv_even.tv_sec > tv_odd.tv_sec || (tv_even.tv_sec == tv_odd.tv_sec && tv_even.tv_usec > tv_odd.tv_usec))
				timestamp_float = tv_even.tv_sec + tv_even.tv_usec/1000000.0;
			else
				timestamp_float = tv_odd.tv_sec + tv_odd.tv_usec/1000000.0;
			outp += sprintf(outp, "%16.4f", timestamp_float );
		}
		if (show_pkg)
			outp += sprintf(outp, "       -");
		if (show_core)
...
void help()
{
	...
	"--Joules	print the energy instead of the power\n"
	"--dump		dump the register values\n"
	"--Summary	print only a summary line\n"
	"--TCC		TCC activation temp override\n"
	"\n"

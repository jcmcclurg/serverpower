...
unsigned int interval_usec = 5000000;	/* set with -i interval_sec */
...
restart:
	...
	while (1) {
		...
		printf("Time: %lf\n",((double)tv_odd.tv_sec) + ((double) tv_odd.tv_usec)/1.0e6);
		...
		printf("Time: %lf\n",((double)tv_even.tv_sec) + ((double) tv_even.tv_usec)/1.0e6);
...
void cmdline(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "+pPsSvi:c:C:m:M:RJT:")) != -1) {
		...
		case 'i':
			...
			interval_usec = (useconds_t) (interval_sec*1.0e6);
			break;
		case 'c':

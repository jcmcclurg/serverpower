...
runQuerySet(){
	...
	local updatePieces=$9

	shift 9
	local queryOtherAttrProb=$1
	local t=$2

	if [ "$t" == "test" ]; then
		local printStats="--print-stats"
	fi
	...
	elif [ "$isQueryTimingImmediate" == 2 ]; then
		local queryTimingType="constant"
	else
		local queryTimingType="none"
	fi
	...
	elif [ "$isUpdateAggregationImmediate" == 2 ]; then
		local updateAggregationTimingType="constant"
	else
		local updateAggregationTimingType="none"
	fi
	...
	#--files=sparkConf/metrics.properties \
	...
	stdbuf -o L -e L \
		...
		--query-other-attr-probability "$queryOtherAttrProb" \
		...
		--update-pieces "$updatePieces" \
		--query-row-select-probability 1.0 \
		--max-preempts 1 \
		--num-segments 1 \
		--queries-preempt \
		$printStats
